...
def process_bert(data, tokenizer, vocab):
    ...
    for index, instance in enumerate(data):
        ...
        if length>512:
            """fetch a sequence of length greater than 512"""
            continue
...
def load_data_bert(config,sort_list,c):
    if sort_list:
        with open('./data/{}'.format(config.dataset["train"]), 'r', encoding='utf-8') as f:
            data = json.load(f)
            train_data=list()
            config.logger.info("\nBefore selection:{}\tTraining data size:{}".format(int(len(data)*c),len(data)))
            for val in  sort_list[:int(len(data)*c)]:
                train_data.append(data[val[0]])
    else:
        with open('./data/{}'.format(config.dataset["train"]), 'r', encoding='utf-8') as f:
            train_data = json.load(f)
        

    with open('./data/{}'.format(config.dataset["valid"]), 'r', encoding='utf-8') as f:
        dev_data = json.load(f)
    with open('./data/{}'.format(config.dataset["test"]), 'r', encoding='utf-8') as f:
    ...
    table = pt.PrettyTable(["path","type",'sentences', 'entities'])
    table.add_row([config.dataset["train"],'train', len(train_data), train_ent_num])
    table.add_row([config.dataset["valid"],'valid', len(dev_data), dev_ent_num])
    table.add_row([config.dataset["test"],'test', len(test_data), test_ent_num])
    config.logger.info("\n{}".format(table))
